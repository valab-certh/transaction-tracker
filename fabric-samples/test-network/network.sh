...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/INCISIVE/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
     ...
     infoln "Creating INCISIVE Identities"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/INCISIVE/msp organizations/fabric-ca/INCISIVE/tls-cert.pem organizations/fabric-ca/INCISIVE/ca-cert.pem organizations/fabric-ca/INCISIVE/IssuerPublicKey organizations/fabric-ca/INCISIVE/IssuerRevocationPublicKey organizations/fabric-ca/INCISIVE/fabric-ca-server.db'
