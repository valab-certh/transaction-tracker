...
// This function returns all the results (data) that were returned with a specific query.
const returneddata = async (req, res) => {
    ...
    try {
        ...
        console.log('\n--> Submit Transaction: ReturnedData, function returns all the results (data) that were returned with a specific query.');
        ...
        console.log('Returned data action submission failed with error: '+error);

        res.status(403).send('Returned data action submission failed with '+error);
